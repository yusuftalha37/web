...
const Store = (() => {
  ...
  function fromDbProduct(r) { return { id: r.id, cat: r.cat, cats: Array.isArray(r.cats) ? r.cats : [], img: r.img, photo: r.photo || "", name: r.name, specs: r.specs || [], price: r.price, stock: r.stock, hit: !!r.hit, authorized: !!r.authorized, bundle: Array.isArray(r.bundle) ? r.bundle : [] }; }
  function toDbProduct(p, sort) { return { id: p.id, cat: p.cat, cats: Array.isArray(p.cats) ? p.cats : [], img: p.img, photo: p.photo || "", name: p.name, specs: p.specs || [], price: p.price, stock: p.stock, hit: !!p.hit, authorized: !!p.authorized, bundle: Array.isArray(p.bundle) ? p.bundle : [], sort: sort == null ? 0 : sort }; }
  function fromDbSlide(r) { return { id: r.id, image: r.image || "", imgPos: r.imgPos || "50% 50%", imgFit: r.imgFit || "cover", art: r.art, title: r.title, subtitle: r.subtitle || "", btnText: r.btnText || "", btnLink: r.btnLink || "urunler.html" }; }
  function toDbSlide(s, sort) { return { id: s.id, image: s.image || "", imgPos: s.imgPos || "center center", imgFit: s.imgFit || "cover", art: s.art, title: s.title, subtitle: s.subtitle || "", btnText: s.btnText || "", btnLink: s.btnLink || "urunler.html", sort: sort == null ? 0 : sort }; }
